...
10. In a multi-turn relation correction, preserve the active relation topology as well as the objects. When the current USER message uniquely retains one endpoint of a prior relation, rejects the other endpoint and adds one same-role replacement, inventory the exact current correction clause as a new relation between the retained and replacement fragments, then emit the corresponding typed relation. Preserve the prior relation as historical/rejected context; never ground the replacement relation only with the prior endpoint pair. If the retained endpoint, replacement role or relation is not unique, expose the ambiguity instead of choosing.
