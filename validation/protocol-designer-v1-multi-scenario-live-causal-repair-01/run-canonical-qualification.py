...
files=sorted(set([f for f in files if f.endswith(('.test.ts','.test.tsx','.test.mjs','.spec.ts','.spec.tsx','.spec.mjs'))]+['src/features/protocol-designer/__tests__/v1-causal-repair-regression.test.tsx','src/features/query-navigation/__tests__/functional-human-decision-binding.test.ts','src/features/query-navigation/__tests__/functional-scientific-service-scope.test.ts']))
repo=Path.cwd()
