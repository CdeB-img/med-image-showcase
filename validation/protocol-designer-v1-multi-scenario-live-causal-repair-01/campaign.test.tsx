// Reused functional convergence harness; only output paths redirected. Oracle and transport unchanged.
...
import { createAdversarialProviderTransport, type ProviderCallWitness } from '../protocol-designer-v1-autonomous-adversarial-stabilization-01/provider-transport';
...
const root = resolve('validation/protocol-designer-v1-multi-scenario-live-causal-repair-01');
const corpusRoot = resolve('validation/protocol-designer-v1-autonomous-adversarial-stabilization-01');
const phase = process.env.NOXIA_CAMPAIGN_PHASE ?? 'DISCOVERY';
const corpus = JSON.parse(readFileSync(resolve(corpusRoot, 'corpus.json'), 'utf8'));
const selected = new Set((process.env.NOXIA_SCENARIOS ?? '').split(',').filter(Boolean));
const resume = JSON.parse(readFileSync(resolve(root, 'mass-resume-state.json'), 'utf8'));
...
for (const scenario of scenarios) it(`${phase} ${scenario.id} — ${scenario.title}`, async () => {
  ...
  const freeze = JSON.parse(readFileSync(resolve(corpusRoot, 'corpus-freeze.json'), 'utf8'));
  expect(sha(readFileSync(resolve(corpusRoot, 'corpus.json'), 'utf8'))).toBe(freeze.corpusSha256);
  const sourceState = { head: git('rev-parse', 'HEAD'), trackedDiffSha256: sha(git('diff', '--', 'src', 'api')), harnessSha256: sha(readFileSync(resolve(root, 'campaign.test.tsx'), 'utf8')), transportSha256: sha(readFileSync(resolve(corpusRoot, 'provider-transport.ts'), 'utf8')), corpusSha256: freeze.corpusSha256 };
      ...
      for (const action of turn.uiActions ?? []) {
        ...
        if (kind.includes('CONFIRM')) {
          ...
          if (!button) {
            ...
            check('DOCUMENT_CURRENT_PROJECT', Boolean(projection && projection.source.projectDigest === current().project?.projectDigest), 'The conversational document command must bind the current Project before returning to the conversation', 'MATERIAL', 'CURRENT_PROJECT_TO_DOCUMENT');
          }
          else {
  ...
  const state = JSON.parse(readFileSync(resolve(root, 'mass-resume-state.json'), 'utf8'));
  ...
  save('mass-resume-state.json', state);
  appendFileSync(resolve(root, 'mass-checkpoint-log.md'), `- ${new Date().toISOString()} — ${phase} ${scenario.id}: ${receipts.length}/${scenario.turns.length} turns; ${anomalies.length} observed anomalies; ${fatal ? 'conversation stopped, cause pending' : 'conversation completed'}. Evidence persisted. New providers=0.\n`);
