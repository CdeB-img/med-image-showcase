...
files=sorted(set(['src/features/protocol-designer/functional-reset/__tests__/v1-product-closure.test.tsx','src/features/document-projection/__tests__/living-document-revision.test.ts']+[f for f in files if f.endswith(('.test.ts','.test.tsx','.test.mjs','.spec.ts','.spec.tsx','.spec.mjs'))]+['src/features/protocol-designer/__tests__/v1-causal-repair-regression.test.tsx','src/features/query-navigation/__tests__/functional-human-decision-binding.test.ts','src/features/query-navigation/__tests__/functional-scientific-service-scope.test.ts']))
repo=Path.cwd()
def source_state():
 tracked=subprocess.check_output(['git','diff','--','src','api'],text=True)
 untracked=subprocess.check_output(['git','ls-files','--others','--exclude-standard','-z','--','src','api'],text=True).split('\0')
 return tracked+json.dumps({f:hashlib.sha256(Path(f).read_bytes()).hexdigest() for f in sorted(untracked) if f and not any(w in f.lower() for w in ['blind','sealed'])},sort_keys=True)
source=source_state()
...
receipt={'exitCode':r.returncode,'seconds':round(time.time()-started,2),'testFiles':len(files),'passed':x.get('numPassedTests'),'failed':x.get('numFailedTests'),'pending':x.get('numPendingTests'),'todo':x.get('numTodoTests'),'success':x.get('success'),'sourceUnchanged':source==source_state(),'providerCalls':0}
