...
import { createClosureOfflineProvider } from "./offline-provider";

const missionRoot = path.resolve("validation/protocol-designer-v1-product-closure-and-demo-readiness-01");
...
const replayFetch = createClosureOfflineProvider(witnesses);
const active = {scenarioId:"PRODUCT_CLOSURE",turnId:"E2E"};
    ...
    server.middlewares.use(async (request, response, next) => {
      ...
      if (url === "/__offline_campaign/status" && request.method === "GET") {
        response.end(JSON.stringify({ provenance: "SYNTHETIC_CONTRACT_FIXTURE", active: active,
          providerWitnesses: witnesses.length, realProviderCalls: 0, scenarios: ["CARDIO", "NEURO"] }));
        return;
      }
      const chunks: Buffer[] = [];
      let size = 0;
      for await (const chunk of request) {
        ...
        size += bytes.length;
        if (size > (url.startsWith("/__offline_campaign/") ? 30_000_000 : 500_000)) { response.statusCode = 413; response.end("{}"); return; }
        chunks.push(bytes);
      }
      ...
      if (url === "/__offline_campaign/artifact") {
        await mkdir(evidenceRoot,{recursive:true});
        await appendFile(path.join(evidenceRoot,"downloaded-artifacts.jsonl"),JSON.stringify({at:new Date().toISOString(),body})+"\n");
        response.end(JSON.stringify({saved:true}));return;
      }
  ...
  plugins: [react(), adversarialOfflineBridge, {
    ...
    transformIndexHtml() { return [{ tag:"script", injectTo:"body", children: `
      document.addEventListener('click', async (event) => {
        ...
        const response = await fetch('/__offline_campaign/receipt', {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({label:document.querySelector("[data-offline-label]").value,visible:document.body.innerText,storage:Object.fromEntries(Object.entries(localStorage))})});
        event.target.textContent=response.ok ? 'Preuve enregistrée' : 'Échec de sauvegarde';
      });
      const originalObjectURL=URL.createObjectURL.bind(URL); URL.createObjectURL=(blob)=>{ if(blob.type.includes('html')) blob.text().then(content=>fetch('/__offline_campaign/artifact',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mimeType:blob.type,content})})); return originalObjectURL(blob); };
      document.addEventListener('DOMContentLoaded',()=>{const label=document.createElement('input');label.setAttribute('aria-label','Nom du constat offline');label.dataset.offlineLabel='1';label.style='position:fixed;bottom:4px;right:190px;z-index:9999;background:white;border:1px solid;width:180px';document.body.append(label);const b=document.createElement('button');b.textContent='Enregistrer la preuve offline';b.dataset.offlineReceipt='1';b.style='position:fixed;bottom:4px;right:4px;z-index:9999;background:white;color:black;border:1px solid;padding:4px';document.body.append(b)});
    ` }]; }
  }],
