...
const result = spawnSync(process.execPath, [
  ...
  "run",
  resolve(import.meta.dirname, "durable-shared-store.test.ts"),
  "--config",
  resolve(import.meta.dirname, "vitest.config.ts"),
  "--reporter=json",
  `--outputFile=${resolve(import.meta.dirname, "beta-closure-tests.json")}`,
...
if (result.status === 0) {
  ...
  writeFileSync(resolve(import.meta.dirname, "beta-closure-cleanup.json"), `${JSON.stringify({
