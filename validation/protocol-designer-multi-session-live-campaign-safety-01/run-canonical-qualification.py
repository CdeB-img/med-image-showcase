...
files=sorted(set([f for f in files if f.endswith(('.test.ts','.test.tsx','.test.mjs','.spec.ts','.spec.tsx','.spec.mjs'))]+['src/features/protocol-designer/__tests__/multi-session-campaign-safety.test.ts']))
repo=Path.cwd()
source=subprocess.check_output(['git','diff','--','src','api','vite.config.ts'],text=True)
(out/f'source-freeze-{phase}.json').write_text(json.dumps({'head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'trackedDiffSha256':hashlib.sha256(source.encode()).hexdigest(),'testFiles':files,'extraTestHashes':{f:hashlib.sha256(Path(f).read_bytes()).hexdigest() for f in files if f.endswith('multi-session-campaign-safety.test.ts')}},indent=2)+'\n')
