...
- **REVIEWER_SIM_1** — `SIMULATED_SEMANTICALLY_EQUIVALENT` — Both candidates retain the active outcome priority and the superseded historical priority.
- **REVIEWER_SIM_2** — `SIMULATED_SEMANTICALLY_EQUIVALENT` — The correction chronology and downstream consequence are unchanged despite distributed nodes.
- **REVIEWER_SIM_3** — `SIMULATED_SEMANTICALLY_EQUIVALENT` — No historical state is reactivated and provenance remains reconstructible in both forms.
...
Both representations encode the same corrected active state and history.

- `SEMANTIC_EQUIVALENCE` → `SIMULATED_SEMANTICALLY_EQUIVALENT` (SEM3B3-SIMDEC-OUTCOME-PRIORITY-CHANGE-SEMANTIC-EQUIVALENCE)
