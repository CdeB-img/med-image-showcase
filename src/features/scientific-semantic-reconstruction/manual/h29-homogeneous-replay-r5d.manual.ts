...
const DIRECTORY = path.resolve(ROOT, "semantic-validation/sem-001r5d");
...
const EXPECTED_CONFIGURATION_NAME = "SEM_LEGACY_R5D";
const EXPECTED_CONFIGURATION_DIGEST = "ke1-273156595eeaae4d";
...
if (!fixture || fixture.turns.length !== 2) throw new Error("SEM001R5D_H29_FIXTURE_INVALID");
...
configurationVerification.decision = configurationPass ? "R5D_CONFIGURATION_VERIFIED_FOR_H29" : "R5D_BLOCKED_BY_CONFIGURATION_DRIFT";
writeJsonAt(path.join(DIRECTORY, "configuration-verification.json"), configurationVerification);
if (!configurationPass) {
  writeJsonAt(RESULT_PATH, { campaign: "SEM-001R5D", caseId: fixture.caseId, decision: "R5D_BLOCKED_BY_CONFIGURATION_DRIFT", configurationVerification, llmOperations: 0, providerRequestStarts: 0 });
...
if (!apiKey) {
  writeJsonAt(RESULT_PATH, { campaign: "SEM-001R5D", caseId: fixture.caseId, decision: "SEM_LEGACY_CLOSURE_BLOCKED_BY_PROVIDER", reason: "GEMINI_API_KEY_MISSING", llmOperations: 0, providerRequestStarts: 0 });
  console.log(JSON.stringify(readJson(RESULT_PATH), null, 2));
  process.exit(3);
}
...
const beforeAttempt = async () => {
  if (!activeOperation) throw new Error("SEM001R5D_PROVIDER_START_OUTSIDE_OPERATION");
  if (activeOperation.requestStarts >= MAX_ATTEMPTS_PER_OPERATION) throw new Error("SEM001R5D_OPERATION_REQUEST_START_BUDGET_EXHAUSTED");
...
const executeOperation = async <T extends { attempts?: SemanticProviderAttempt[] }>(
  ...
  operationSequence += 1;
  if (operationSequence > 6) throw new Error("SEM001R5D_H29_LLM_OPERATION_BUDGET_EXCEEDED");
...
const result: any = {
  ...
  configurationName: EXPECTED_CONFIGURATION_NAME,
  configurationDigest: EXPECTED_CONFIGURATION_DIGEST,
  chain: "HOMOGENEOUS_R5D_NO_PRIOR_RESULT_REUSE",
...
try {
  for (let turnIndex = 0; turnIndex < fixture.turns.length; turnIndex += 1) {
    ...
    const request: SemanticReconstructionRequest = {
      schemaVersion: SCIENTIFIC_SEMANTIC_SCHEMA_VERSION,
      sessionId: `sem-001r5d:${fixture.caseId}`,
      language: "fr",
      messages: [...messages],
    ...
    if (!critique.accepted) {
      ...
      throw new Error(`SEM001R5D_CRITIC_NOT_ACCEPTED:${critique.terminalReason}`);
    }
    const finalCritic = critique.critics.at(-1);
    const finalCriticCallId = critique.callIds.at(-1);
    if (!finalCritic || !finalCriticCallId) throw new Error("SEM001R5D_CRITIC_RESULT_MISSING");
  ...
  if (!semanticModel) throw new Error("SEM001R5D_CANONICAL_MODEL_MISSING");
