...
const DIRECTORY = path.resolve(ROOT, "semantic-validation/sem-001r5p");
const CAMPAIGN = "SEM-001R5P-CLOSURE";
...
const EXPECTED_CONFIGURATION_NAME = "SEM_LEGACY_R5P";
const EXPECTED_CONFIGURATION_DIGEST = "ke1-2ac79d2f15fe20bb";
...
if (PREPARE === RUN) throw new Error("SEM001R5P_EXACTLY_ONE_PHASE_REQUIRED");
...
const configurationDecision = checks.every((item) => item.pass) ? "R5P_CLOSURE_CONFIGURATION_VERIFIED" : "R5P_CLOSURE_BLOCKED_BY_CONFIGURATION_DRIFT";
...
if (configurationDecision !== "R5P_CLOSURE_CONFIGURATION_VERIFIED") {
...
if (!exists(activationPath)) writeJsonAt(activationPath, {
  campaign: CAMPAIGN,
  activatedAt,
  decision: "R5P_QUALIFYING_FREEZE_ACTIVATED_FOR_FULL_HOLDOUT",
...
if (exists(manifestPath)) {
  ...
  if (manifest.configurationDigest !== configurationDigest) throw new Error("SEM001R5P_CLOSURE_MANIFEST_CONFIGURATION_DRIFT");
} else {
  const startedAt = new Date().toISOString();
  manifest = {
    campaignId: `sem-001r5p-closure-${startedAt.replace(/[:.]/g, "-")}-${configurationDigest}`,
    campaign: CAMPAIGN,
  ...
  if (!h29Fixture) throw new Error("SEM001R5P_H29_FIXTURE_MISSING");
  const seededH29 = {
    campaign: CAMPAIGN,
...
if (PREPARE) {
  console.log(JSON.stringify({ decision: "R5P_QUALIFYING_FREEZE_ACTIVATED_READY_FOR_FULL_HOLDOUT", campaignId: manifest.campaignId, configurationDigest, holdout: "5/30 TERMINAL", evaluated: "1/26 COMPLETE", governedUnresolved: [...GOVERNED_UNRESOLVED_CASE_IDS], compatibleCheckpoint: "SEM-H29", resumeAt: "SEM-H01", providerCalls: 0, callsAvoided: h29Result.providerRequestStarts }, null, 2));
  process.exit(0);
}
...
const assertFrozenConfiguration = () => {
  ...
  if (changed.length || logicalDigest(freeze.configurationMaterial.thresholds) !== thresholdsDigest || logicalDigest(freeze.configurationMaterial) !== configurationDigest || freeze.configurationMaterial?.provider?.model !== MODEL_ID) {
    throw new Error(`SEM001R5P_CLOSURE_CONFIGURATION_MUTATED_AFTER_MANIFEST:${changed.join(",") || "THRESHOLD_OR_MODEL"}`);
  }
};
...
const runId = `sem-001r5p-closure-${new Date().toISOString().replace(/[:.]/g, "-")}`;
...
for (const fixture of HOLDOUT_CASES) {
  ...
  if (results.some((item) => item.caseId === fixture.caseId && ["COMPLETE", "UNRESOLVED_BY_LEGACY_MODEL"].includes(item.finalStatus) && item.configurationDigest === configurationDigest)) {
    console.log(`SEM-R5P-CLOSURE ${fixture.caseId} RESUMED_TERMINAL`);
    continue;
  }
  const startedAt = new Date().toISOString();
  const result: any = {
  ...
  checkpointResults();
  writeJson("failure-ledger.json", { campaign: CAMPAIGN, campaignId: manifest.campaignId, failures });
  console.log(`SEM-R5P-CLOSURE ${fixture.caseId} ${result.finalStatus} evaluated=${results.filter((item) => item.finalStatus === "COMPLETE").length}/26 terminal=${results.filter((item) => ["COMPLETE", "UNRESOLVED_BY_LEGACY_MODEL", "FAILED", "PROVIDER_CAPACITY_FAILURE"].includes(item.finalStatus)).length}/30 starts=${providerDiagnostics.attempts.length}`);
