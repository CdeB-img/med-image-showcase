...
describe("UX-001 Part 4 — 50 core closure gates", () => {
  ...
  it("UX-CLOSE-C39 Manual navigation does not rerank QRY", () => { const fixture = renderWorkspace(); const before = JSON.stringify(fixture.navigation); fireEvent.click(screen.getAllByRole("button", { name: "Ouvrir" })[0]); expect(JSON.stringify(fixture.navigation)).toBe(before); });
  ...
  it("UX-CLOSE-C44 Desktop acceptance passes", () => { renderWorkspace(); expect(screen.getByRole("main").parentElement?.className).toContain("xl:grid-cols"); });
  it("UX-CLOSE-C45 Tablet acceptance passes", () => { renderWorkspace(); expect(screen.getByRole("heading", { name: "Dimensions de l’étude" }).nextElementSibling?.nextElementSibling?.className).toContain("sm:grid-cols-2"); });
