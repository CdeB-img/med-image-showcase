...
describe("UX-001 Part 2 — product surface", () => {
  ...
  it("UX2-UI-C01 presents a Project-centric workspace", () => { renderWorkspace(); expect(screen.getByRole("heading", { name: "Votre étude, au même endroit" })).toBeInTheDocument(); expect(screen.getByText(/Question :/)).toBeInTheDocument(); });
  ...
  it("UX2-UI-C03 shows attention, domains, documents", () => { renderWorkspace(); expect(screen.getByRole("heading", { name: "À regarder maintenant" })).toBeInTheDocument(); expect(screen.getByRole("heading", { name: "Dimensions de l’étude" })).toBeInTheDocument(); expect(screen.getByRole("heading", { name: "Documents" })).toBeInTheDocument(); });
  ...
  it("UX2-UI-C07 domain navigation emits only a target ref", () => { const value = renderWorkspace(); fireEvent.click(screen.getAllByRole("button", { name: "Ouvrir" })[0]); expect(value.openSurface).toHaveBeenCalledTimes(1); expect(typeof value.openSurface.mock.calls[0][0]).toBe("string"); });
  it("UX2-UI-C08 document navigation emits only a target ref", () => { const value = renderWorkspace(); fireEvent.click(screen.getAllByRole("button", { name: "Voir la projection" })[0]); expect(value.openDocument).toHaveBeenCalledTimes(1); });
  ...
  it("UX2-UI-C10 uses 44px-equivalent minimum interactive targets", () => { renderWorkspace(); expect(screen.getByRole("button", { name: "Expert" }).className).toContain("min-h-10"); expect(screen.getAllByRole("button", { name: "Ouvrir" })[0].className).toContain("min-h-11"); });
  it("UX2-UI-C11 has responsive one/two/three-column foundations", () => { renderWorkspace(); const main = screen.getByRole("main"); expect(main.parentElement?.className).toContain("xl:grid-cols"); });
