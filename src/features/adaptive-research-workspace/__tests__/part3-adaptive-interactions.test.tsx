...
describe("UX-001 Part 3 — QRY and response contracts", () => {
  ...
  it("UX3-QRY-C04 why-now is shown", () => { const projection = presentable(); render(<WorkspaceNextActionInteraction projection={projection} currentProjectVersion={projection.projectVersion} currentSourceStateDigest={projection.sourceStateDigest} />); expect(screen.getByText(/Cette information change la branche/)).toBeInTheDocument(); });
...
describe("UX-001 Part 3 — product interaction surface", () => {
  ...
  it("UX3-UI-C02 free-text submit produces a handoff, not Project write", () => { const projection = presentable(); const callback = vi.fn(); render(<WorkspaceNextActionInteraction projection={projection} currentProjectVersion={projection.projectVersion} currentSourceStateDigest={projection.sourceStateDigest} onOwnerHandoff={callback} />); fireEvent.change(screen.getByLabelText("Votre réponse"), { target: { value: "six mois" } }); fireEvent.click(screen.getByRole("button", { name: "Transmettre comme contribution candidate" })); expect(callback.mock.calls[0][0]).toMatchObject({ projectWriteAuthorized: false, state: "INTERPRETATION_PENDING" }); });
  ...
  it("UX3-UI-C06 stale response is disabled and explained", () => { const projection = presentable(); render(<WorkspaceNextActionInteraction projection={projection} currentProjectVersion="next" currentSourceStateDigest={projection.sourceStateDigest} />); const input = screen.getByLabelText("Votre réponse"); fireEvent.change(input, { target: { value: "texte conservé" } }); expect(screen.getByRole("button", { name: "Transmettre comme contribution candidate" })).toBeDisabled(); expect(screen.getByRole("alert")).toHaveTextContent(/projet a changé/); expect(input).toHaveValue("texte conservé"); });
