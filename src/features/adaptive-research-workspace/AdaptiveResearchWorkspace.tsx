...
const documentStateLabel = {
  GENERATABLE: "Projection générable",
  PARTIALLY_GENERATABLE: "Structure partiellement disponible",
  ...
  return <section className="rounded-3xl border bg-background p-4 shadow-sm sm:p-6" aria-labelledby="adaptive-workspace-title" data-testid="adaptive-research-workspace">
    ...
    <div className="mt-6 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(19rem,0.75fr)]">
      <main className="min-w-0 space-y-6">
        ...
        <section aria-labelledby="workspace-documents-title"><h3 id="workspace-documents-title" className="text-xl font-semibold">Documents</h3><p className="mt-1 text-sm text-muted-foreground">Chaque projection suit sa propre générabilité DOC et la version du Research Project qui la fonde.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{primaryDocuments.map((document) => <article key={document.projection} className="rounded-2xl border p-4"><Status state={document.stale ? "STALE" : document.state === "GENERATABLE" ? "ADOPTED" : document.state === "PARTIALLY_GENERATABLE" ? "WARNING" : document.state === "NOT_APPLICABLE" ? "NOT_APPLICABLE" : "NOT_GENERATABLE"} /><h4 className="mt-3 font-semibold">{document.projection}</h4><p role="status" className="mt-2 text-sm font-medium">{documentStateLabel[document.state]}</p><p className="mt-1 text-xs text-muted-foreground">Basée sur la version {document.projectVersion} du Research Project.</p>{document.missing.length ? <div className="mt-2 text-sm text-muted-foreground"><p>Cette projection attend :</p><ul className="mt-1 list-disc pl-5">{document.missing.slice(0, mode === "EXPERT" ? undefined : 3).map((item) => <li key={item}>{item}</li>)}</ul></div> : <p className="mt-2 text-sm text-muted-foreground">Projection disponible selon l’état DOC courant.</p>}<div className="mt-3 flex flex-wrap gap-2">{document.missing.length > 0 && onOpenSurface && <button type="button" onClick={() => onOpenSurface(document.targetRef)} className="min-h-11 rounded-lg border px-3 py-2 text-sm">Traiter ce blocage</button>}{document.actionAvailability.preview && onOpenDocument && <button type="button" aria-label="Voir la projection" onClick={() => onOpenDocument(document.targetRef)} className="min-h-11 rounded-lg border px-3 py-2 text-sm">Prévisualiser</button>}</div>{visibility.versions && <p className="mt-3 break-all text-xs text-muted-foreground">Owner : {document.owner} · Source : {document.sourceRef} · Générabilité : {document.generatabilitySource} · Droit commercial V1 : non applicable</p>}</article>)}</div></section>
      </main>
