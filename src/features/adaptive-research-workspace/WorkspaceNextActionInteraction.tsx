...
export default function WorkspaceNextActionInteraction({ projection, currentProjectVersion, currentSourceStateDigest, onOwnerHandoff, onOpenTarget, onChooseNavigationPreference }: WorkspaceNextActionInteractionProps) {
  ...
  return <section className="rounded-2xl border border-primary/40 bg-card p-5 shadow-sm" data-testid="workspace-next-action" aria-labelledby="workspace-next-action-title">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-primary">Prochaine action scientifique</p><h3 id="workspace-next-action-title" className="mt-2 text-2xl font-bold">{projection.summary.actionLabel ?? "Choisir une action"}</h3></div><span className="rounded-full border bg-muted px-2.5 py-1 text-xs">{projection.status.replace(/_/g, " ").toLocaleLowerCase("fr-FR")}</span></div>
    <p className="mt-3 text-sm"><strong>Pourquoi :</strong> {projection.summary.reason}</p>
    <p className="mt-3 text-sm"><strong>Pourquoi maintenant :</strong> {projection.summary.whyNow}</p>
    <p className="mt-2 text-sm"><strong>Influence :</strong> {[...projection.summary.affectedDecisionRefs, ...projection.summary.affectedBranchRefs].join(" · ") || "navigation courante"}</p>
    {projection.summary.deferAllowed && <p className="mt-2 text-sm text-muted-foreground"><strong>Si différée :</strong> {projection.summary.deferConsequence ?? "le besoin reste ouvert"}</p>}
    ...
    {presentation && action && <div className="mt-5 rounded-xl bg-muted/50 p-4" aria-describedby="workspace-question-context"><p className="font-semibold">{presentation.intent}</p><p id="workspace-question-context" className="mt-2 text-sm text-muted-foreground">Cette réponse concerne {presentation.targetRef}. UNKNOWN, différer et refuser restent des réponses légitimes.</p>
      {presentation.expectedAnswerKind === "FREE_TEXT" && <div className="mt-4"><label htmlFor="workspace-free-response" className="text-sm font-medium">Votre réponse</label><textarea id="workspace-free-response" value={rawResponse} onChange={(event) => setRawResponse(event.target.value)} className="mt-2 min-h-28 w-full rounded-lg border bg-background p-3" aria-describedby="workspace-free-response-help" /><p id="workspace-free-response-help" className="mt-1 text-xs text-muted-foreground">Le texte reste une contribution brute ; il ne devient jamais directement la vérité du Project.</p><button type="button" disabled={!rawResponse.trim() || freshness.status !== "CURRENT"} onClick={() => respond("ANSWER")} className="mt-3 min-h-11 rounded-lg bg-primary px-4 py-2 text-primary-foreground disabled:opacity-50">Transmettre comme contribution candidate</button></div>}
      {["SINGLE_OPTION", "MULTIPLE_OPTIONS", "HUMAN_REVIEW_DECISION"].includes(presentation.expectedAnswerKind) && <fieldset className="mt-4"><legend className="text-sm font-medium">Options disponibles</legend><p className="mt-1 text-xs text-muted-foreground">Aucune option n’est présélectionnée.</p><div className="mt-2 grid gap-2">{presentation.knownOptions.map((option) => <label key={option} className="flex min-h-11 items-center gap-3 rounded-lg border bg-background p-3"><input type="radio" name="workspace-option" checked={selectedOptionRef === option} onChange={() => setSelectedOptionRef(option)} /><span>{option}</span></label>)}</div><button type="button" disabled={!selectedOptionRef || freshness.status !== "CURRENT"} onClick={() => respond("ANSWER")} className="mt-3 min-h-11 rounded-lg bg-primary px-4 py-2 text-primary-foreground disabled:opacity-50">Soumettre ce choix à la décision humaine</button></fieldset>}
    ...
    {submittedHandoff && <p className="mt-2 text-xs text-muted-foreground">Destination : {submittedHandoff.route.destination.replace(/_/g, " ").toLocaleLowerCase("fr-FR")}. Écriture Project : non.</p>}
  </section>;
}
