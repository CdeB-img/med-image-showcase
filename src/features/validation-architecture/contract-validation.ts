...
export const validateValidationRun = (run: Readonly<ValidationRun>): ValidationContractValidationResult => {
  ...
  const missingSourceExpected = run.applicability?.status === "MISSING_SOURCE";
  const missingTargetExpected = run.applicability?.status === "MISSING_TARGET";
  if (!run.sourceArtifactRef && !missingSourceExpected) diagnostics.push(error("VAL_SOURCE_ARTIFACT_MISSING", "sourceArtifactRef", "Source reference is required unless applicability records MISSING_SOURCE."));
  if (!run.targetArtifactRef && !missingTargetExpected) diagnostics.push(error("VAL_TARGET_ARTIFACT_MISSING", "targetArtifactRef", "Target reference is required unless applicability records MISSING_TARGET."));
  if (run.sourceArtifactRef && !run.sourceArtifactRef.version) diagnostics.push(error("VAL_SOURCE_VERSION_MISSING", "sourceArtifactRef.version", "Source version is required."));
  if (run.targetArtifactRef && !run.targetArtifactRef.version) diagnostics.push(error("VAL_TARGET_VERSION_MISSING", "targetArtifactRef.version", "Target version is required."));
  if (run.sourceArtifactRef && !run.sourceArtifactRef.contentDigest) diagnostics.push(error("VAL_SOURCE_DIGEST_INVALID", "sourceArtifactRef.contentDigest", "Source digest is required."));
  if (run.targetArtifactRef && !run.targetArtifactRef.contentDigest) diagnostics.push(error("VAL_TARGET_DIGEST_INVALID", "targetArtifactRef.contentDigest", "Target digest is required."));
  if ((run.sourceArtifactRef && !hasAdapterFor(run.sourceArtifactRef.artifactType)) || (run.targetArtifactRef && !hasAdapterFor(run.targetArtifactRef.artifactType))) diagnostics.push(error("VAL_ADAPTER_NOT_FOUND", "artifactRefs", "A run references an artifact type without a registered read-only adapter."));
  if (run.applicability?.status === "APPLICABLE" && !run.adapterVersions.length) diagnostics.push(error("VAL_ADAPTER_NOT_FOUND", "adapterVersions", "At least one adapter identity is required for an applicable run."));
  ...
  if (run.semanticReviewRequests.some((item) => item.providerPolicy !== "DISABLED_BY_DEFAULT")) diagnostics.push(error("VAL_SEMANTIC_REVIEW_NOT_EXECUTABLE_IN_PART2", "semanticReviewRequests", "Part 3 may prepare requests only while provider execution remains disabled."));
