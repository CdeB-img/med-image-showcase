...
export default function ImagingStudyDesignerView({ session, onChange, onReturnToScientificThinking, onExploreKnowledge, onProjectConstructionHandoff }: Props) {
  ...
  return <div className="mt-8 min-w-0" data-testid="imaging-study-designer">
    ...
    <div className="mt-6" aria-live="polite" aria-atomic="false">
      ...
      {stage === 7 && <>
        <h3 className="text-2xl font-bold">Stratégie Imaging</h3><p className="mt-1 text-muted-foreground">Alternatives, compromis, décisions humaines et handoff vers Project Construction.</p>
        ...
        <Card className="mt-5"><h4 className="text-lg font-semibold">Handoff Project Construction</h4><div className="mt-2 flex flex-wrap gap-2"><Pill warning={result.projectConstructionHandoff.status === "NOT_READY"}>{result.projectConstructionHandoff.status}</Pill><Pill>{result.projectConstructionHandoff.blockedBy.length} blocage(s)</Pill></div><p className="mt-3 text-sm">Le handoff contient Question, Objectifs, Hypothèses, stratégie Imaging, QA, Variables, limites, gaps, alternatives, décisions et provenance. Il exclut dimensionnement, budget, CRF, plan réglementaire et protocole final.</p><button type="button" disabled={result.projectConstructionHandoff.status !== "FROZEN_BY_HUMAN"} onClick={onProjectConstructionHandoff} className="mt-4 rounded-lg bg-primary px-4 py-2 text-primary-foreground disabled:opacity-50">Passer à Project Construction</button></Card>
      </>}
    </div>
