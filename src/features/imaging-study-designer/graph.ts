...
export const buildImagingDecisionGraph = (input: ImagingDesignInput, material: GraphMaterial): ImagingDecisionGraph => {
  ...
  material.equipmentAssessment.filter((item) => item.compatibility === "INCOMPATIBLE").forEach((item) => broken("EQUIPMENT_INCOMPATIBLE_WITH_REQUIRED_MODALITY", "Équipement incompatible avec la modalité requise", [item.equipmentId, item.acquisitionId], "La branche d’acquisition nécessaire est impossible avec l’équipement déclaré."));
