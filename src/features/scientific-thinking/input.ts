...
export const buildScientificThinkingInput = (
  ...
  const result: ScientificThinkingInput = {
    ...
    scientificIntent: {
      ...
      semanticModelRef: intent.semanticSnapshot?.semanticModelId ?? intent.interpretationContributionSnapshot?.contributionId,
      semanticModelDigest: intent.semanticSnapshot?.semanticModelDigest ?? intent.interpretationContributionSnapshot?.contributionDigest,
    },
    researchContext: {
    ...
    knowledge: {
      ownerResultRef: knowledgeResult ? `${knowledgeResult.resultId}@${knowledgeResult.resultRevision}` : null,
      ...
      assertionRefs: uniqueSorted(knowledgeResult?.applicableAssertions.map((item) => item.stableId) ?? []),
      documentaryStatementRefs: uniqueSorted(knowledgeResult?.documentaryStatements.map((item) => item.statementId) ?? []),
      evidenceRefs: uniqueSorted(knowledgeResult?.evidence.map((item) => item.evidenceId) ?? []),
      applicability: (knowledgeResult?.applicableAssertions ?? []).map((item) => ({ assertionRef: item.stableId, status: item.applicability })),
      contradictionRefs: uniqueSorted(knowledgeResult?.controversies.map((item) => item.conflictId) ?? []),
