...
/** Project the native parser, as the persistent-delta path already does.
 * The provider must spell every field; historical stored candidates retain
 * their optional fields and still use the unchanged native validation rules.
 */
export const workingDraftProviderSchema = (inputDigest: string, turns: ProductBridgeRequest["conversation"]["turns"] = []) => {
  // Constrain the existing quote/turn pair to immutable source spans. The model
...
export const prepareWorkingDraftRequest = (request: ProductBridgeRequest) => {
  ...
  const context = JSON.stringify({ ...base, contextDigest: inputDigest,
    // Keep the quote and the exact output-contract key on the same source object.
    // This is the existing transcript, not a second provenance store.
    RECENT_CONVERSATION: request.conversation.turns.map(turn => ({
      ref: turn.turnId, ...(turn.role === "USER" ? { sourceTurnRef: turn.turnId } : {}),
...
RECUEIL : représente séparément par des atomes CANONICAL_VARIABLE les données déjà impliquées par les décisions explicites : mesure du critère principal et ses entrées, prédicteurs retenus, vérifications d'éligibilité, événements intercurrents et évaluabilité. Un ENDPOINT, une CONSTRAINT ou une ANALYSIS_SPECIFICATION décrit une règle, pas à lui seul les variables à recueillir ; variableRoles sur cette règle ne crée aucune variable. Une variable = une donnée définissable, jamais un bloc réunissant plusieurs mesures indépendantes. Relie ces variables aux décisions et visites correspondantes ; garde les unités, méthodes, sources et règles non décidées ouvertes. N'ajoute aucune mesure nouvelle, covariable non retenue ou arbitrage scientifique pour compléter le recueil. Les atomes nécessaires à cette séparation priment sur la cible indicative du nombre d'atomes, avec des justifications compactes.
CONTENU ADOPTABLE : seul content d'un atome devient une décision présentée à l'humain puis un fait Project utilisable par DOC. rationale explique cette décision ; understanding et les citations en documentent le contexte, sans devenir des faits adoptés. Ne laisse donc aucune condition, négation, exception, règle d'analyse ou incertitude explicite uniquement dans ces champs. Distingue une donnée à recueillir et la règle qui gouverne son interprétation : conserve chacune dans le content du type natif approprié. Chaque content doit être scientifiquement autonome. Chaque choix explicitement laissé ouvert doit rester représenté comme OPEN_DECISION, même si plusieurs inconnus ont été formulés dans la même phrase. Avant émission, vérifie la couverture de chaque décision USER courante et de ses conditions dans les content, sans promouvoir les recommandations voisines ni recopier le transcript comme substitut des atomes.
PORTÉE DES OPTIONS : options[].atomRefs désigne les choix propres à cette alternative, jamais les décisions déjà exprimées par USER ni les données sources et critères indépendants que l'option utilise. Les prérequis se représentent avec dependsOn sur l'atome de décision de l'option. Une stratégie encore ouverte ne rend pas optionnels ses données sources, ses critères ou leurs règles déjà retenues. Sépare le socle stable des alternatives nouvelles ; laisse un arbitrage sans recommandation si son choix manque, sans masquer ce socle dans ses options. Un arbitrage résolu par USER ne demeure pas un choix alternatif ouvert sur les mêmes décisions.
${STUDY_PROPOSAL_MANDATE}
PROVENANCE DES CHOIX EXPLICITES : recopie sourceTurnRef et quote depuis le MÊME objet USER de RECENT_CONVERSATION. quote est un passage de son content, jamais celui d'un autre tour. Le tour qui conserve ou actualise le brouillon n'est pas automatiquement le tour qui a exprimé chaque décision. Pour un choix antérieur inchangé, conserve sa source antérieure ; pour une correction, cite le tour qui exprime cette correction. Vérifie chaque paire sourceTurnRef/quote avant émission. Une recommandation de NOXIA sans confirmation USER reste proposée ; ne fabrique aucune citation pour la rendre explicite.
Le schéma fournit les citations exactes admissibles pour chaque sourceTurnRef : sélectionne le passage qui porte le choix, même s'il mentionne aussi d'autres éléments. Ne reformule et n'abrège jamais ces citations, y compris pour un choix laissé ouvert. Le texte source reste inchangé.
...
export const acceptWorkingDraftUpdate = (raw: unknown, request: ProductBridgeRequest) => {
  ...
  update.explicitDecisions = update.explicitDecisions.map(decision => {
    const sources = request.conversation.turns.filter(t => t.turnId === decision.sourceTurnRef);
    const source = sources.length === 1 && sources[0]!.role === "USER" ? sources[0] : null;
  ...
  // An option label cannot silently demote a current explicit decision. Reject
  // the producer's grouping; never select its alternative or weaken closure.
