...
const enrichLegacyProjection = (
  ...
  const visits = [
    ...snapshot.temporalQualifications.map((qualification) => ({
      ...
      temporalRole: qualification.anchor.reference.status !== "UNKNOWN" && qualification.anchor.direction === "AFTER" ? "FOLLOW_UP" as const : "SINGLE_ASSESSMENT" as const,
      timingValue: anchorText(qualification.anchor),
      timingStatus: qualification.anchor.reference.status !== "UNKNOWN" ? "KNOWN" as const : "SCIENTIFIC_WINDOW_TO_DEFINE" as const,
      justification: qualification.anchor.reference.status !== "UNKNOWN" ? "Qualification temporelle canonique référencée." : `Référence temporelle explicitement inconnue : ${qualification.anchor.reference.unresolvedReason}.`,
      hypothesisIds: [],
    ...
    ...snapshot.expectedVariableOccasions.map((occasion) => ({
      ...
      label: "EXPECTED_AT",
      temporalRole: occasion.anchor.reference.status !== "UNKNOWN" && occasion.anchor.direction === "AFTER" ? "FOLLOW_UP" as const : "SINGLE_ASSESSMENT" as const,
      timingValue: anchorText(occasion.anchor),
      timingStatus: occasion.anchor.reference.status !== "UNKNOWN" ? "KNOWN" as const : "SCIENTIFIC_WINDOW_TO_DEFINE" as const,
      justification: occasion.anchor.reference.status !== "UNKNOWN" ? "Occasion attendue canonique référencée." : `Référence temporelle explicitement inconnue : ${occasion.anchor.reference.unresolvedReason}.`,
      hypothesisIds: [],
  ...
  return {
    ...
    temporalStructure: {
      ...
      anchor: snapshot.temporalQualifications.find((qualification) => qualification.anchor.reference.status !== "UNKNOWN")?.anchor.relativeEventLabel ?? null,
