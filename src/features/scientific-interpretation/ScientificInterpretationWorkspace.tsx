...
export default function ScientificInterpretationWorkspace({ onOpenStructuredProject, onResumeStructuredProject }: Props) {
  ...
  return <main id="demo-main" className="min-h-screen bg-background text-foreground">
    <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">{onResumeStructuredProject && <button onClick={onResumeStructuredProject} className="rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground">Reprendre</button>}<button aria-expanded={expertOpen} onClick={() => setExpertOpen((current) => !current)} className="rounded-lg border px-3 py-2 text-sm">{expertOpen ? "Masquer l’audit" : "Audit / mode expert"}</button><AlertDialog><AlertDialogTrigger asChild><button className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm"><RotateCcw className="h-4 w-4" /> Réinitialiser la conversation</button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Réinitialiser cette conversation ?</AlertDialogTitle><AlertDialogDescription>Les messages, Contributions et corrections de cet espace local seront supprimés. Les autres données du navigateur ne seront pas touchées.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Annuler</AlertDialogCancel><AlertDialogAction onClick={reset}>Supprimer cette conversation</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div>
      </header>
