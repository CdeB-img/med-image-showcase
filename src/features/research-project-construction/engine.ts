...
const buildFeasibility = (input: ResearchProjectConstructionInput, population: PopulationDesign, endpoints: EndpointCandidate[], designs: StudyDesignCandidate[]): ResearchProjectDesignResult["feasibilityAssessment"] => {
  const equipmentStatus = input.sourceHandoffs.imaging.equipmentCompatibilityStatus;
  const technicalStatus: SpecializedEvaluationState = !input.imagingDesignResult
    ? input.sourceHandoffs.imaging.status === "NOT_APPLICABLE" ? "NOT_APPLICABLE" : "BLOCKED"
    : equipmentStatus === "INCOMPATIBLE" ? "BLOCKED"
      : equipmentStatus === "TECHNICAL_COMPATIBILITY_CONFIRMED" ? "READY_WITH_LIMITATIONS"
        : "PARTIAL";
  const dimensions = [
    ...
    ["MEASUREMENT_FEASIBILITY", input.sourceHandoffs.imaging.projectHandoffReadiness === "PROJECT_HANDOFF_READY" || endpoints.some((item) => item.measurementMethod) ? "READY_WITH_LIMITATIONS" : "PARTIAL", [`${endpoints.length} Critère(s) candidat(s)`], endpoints.length ? [] : ["Aucune mesure opérationnalisée"], null],
    ...
    ["TECHNICAL_FEASIBILITY", technicalStatus, input.imagingDesignResult ? ["Stratégie Imaging gelée", `Compatibilité : ${equipmentStatus ?? "UNKNOWN"}`] : [], input.sourceHandoffs.imaging.status === "REQUIRED_BUT_NOT_READY" ? ["Handoff Imaging gelé requis"] : input.imagingDesignResult && equipmentStatus !== "TECHNICAL_COMPATIBILITY_CONFIRMED" ? ["Compatibilité technique à qualifier avant protocole exécutable"] : [], "Imaging Engine"],
...
export const executeResearchProjectConstruction = (input: ResearchProjectConstructionInput, controls: ResearchProjectControls = {}): ResearchProjectDesignResult => {
  ...
  const imagingContribution: ResearchProjectDesignResult["imagingContribution"] = input.imagingDesignResult ? { applicability: "APPLICABLE", resultRef: input.imagingDesignResult.resultId, variableIds: input.imagingDesignResult.imagingVariables.map((item) => item.variableId), acquisitionRefs: input.imagingDesignResult.acquisitionStrategies.map((item) => item.acquisitionId), qualityRefs: input.imagingDesignResult.qualityStrategy.map((item) => item.ruleId), limitations: uniqueSorted([...input.imagingDesignResult.limitations, ...input.imagingDesignResult.projectConstructionHandoff.limitations]), projectHandoffReadiness: input.imagingDesignResult.projectConstructionHandoff.projectHandoffReadiness, equipmentCompatibilityStatus: input.imagingDesignResult.projectConstructionHandoff.equipmentCompatibilityStatus, executableProtocolReadiness: input.imagingDesignResult.projectConstructionHandoff.executableProtocolReadiness, requiredFutureReviews: input.imagingDesignResult.projectConstructionHandoff.requiredFutureReviews }
    : input.sourceHandoffs.imaging.status === "NOT_APPLICABLE" ? { applicability: "NOT_APPLICABLE", resultRef: null, variableIds: [], acquisitionRefs: [], qualityRefs: [], limitations: [], projectHandoffReadiness: null, equipmentCompatibilityStatus: "NOT_APPLICABLE", executableProtocolReadiness: null, requiredFutureReviews: [] }
      : { applicability: "REQUIRED_BUT_NOT_READY", resultRef: null, variableIds: [], acquisitionRefs: [], qualityRefs: [], limitations: ["Handoff Imaging gelé requis avant construction."], projectHandoffReadiness: input.sourceHandoffs.imaging.projectHandoffReadiness, equipmentCompatibilityStatus: input.sourceHandoffs.imaging.equipmentCompatibilityStatus, executableProtocolReadiness: input.sourceHandoffs.imaging.executableProtocolReadiness, requiredFutureReviews: [] };
