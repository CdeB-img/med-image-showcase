...
${projection.sections.map((section) => `<section id="${escapeHtml(section.sectionId)}"><h2>${section.order}. ${escapeHtml(section.title)}</h2><p><strong>Statut :</strong> ${section.status} · <strong>Applicabilité :</strong> ${section.applicability} · <strong>TMP :</strong> ${section.templateStatus ?? "LEGACY"}</p>${section.templateNodeIds.length ? `<h3>Nœuds TMP</h3>${list(section.templateNodeIds)}` : ""}${section.futureReason ? `<p><strong>Dépendance future :</strong> ${escapeHtml(section.futureReason)}</p>` : ""}${section.statusReasons.length ? `<h3>Règles de décision</h3>${list(section.statusReasons)}` : ""}${section.blocks.map((block) => `<div data-commitment="${block.commitment}"><h3>${escapeHtml(block.label ?? block.kind)}</h3>${list(block.items)}</div>`).join("")}${section.unknowns.length ? `<h3>Inconnues</h3>${list(section.unknowns)}` : ""}${section.limitations.length ? `<h3>Limitations</h3>${list(section.limitations)}` : ""}${section.contradictions.length ? `<h3>Contradictions</h3>${list(section.contradictions)}` : ""}${section.requirementIds.length ? `<h3>Exigences REG-001</h3>${list(section.requirementIds)}` : ""}${section.patternIds.length ? `<h3>Patterns DOC-002</h3>${list(section.patternIds)}` : ""}${section.humanDecisionIds.length ? `<h3>Décisions humaines liées</h3>${list(section.humanDecisionIds)}` : ""}<h3>Provenance</h3>${list(section.provenanceRefs)}</section>`).join("\n")}
