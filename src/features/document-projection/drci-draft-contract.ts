...
MÉTHODE : transforme les décisions en parcours compréhensible de sélection, consentement si applicable, visites, interventions ou observations, collecte, contrôle de qualité, dérivation et analyse. Respecte les temporalités, rôles et sources adoptés. Les choix ouverts de technique, lecture, aveuglement, calendrier, seuils et conduite clinique restent ouverts. Une exclusion analytique ne supprime pas automatiquement la participation ni les données sources. Décris les besoins de standardisation sans inventer une procédure.
STATISTIQUES : restitue les estimands, populations d'analyse, méthodes, ajustements et statuts principal/secondaire/exploratoire réellement adoptés. N'introduis aucune covariable refusée ni méthode par défaut. Explique les estimations et incertitudes compatibles avec la méthode retenue sans imposer un niveau absent. Sépare inclus, analysables et données non évaluables ; les exclusions proviennent uniquement du Project. Décris les diagnostics pertinents sans choisir transformation, seuil de retrait, imputation ou stratégie de données manquantes non adoptés. Un N pragmatique n'est jamais un calcul de puissance : effet, précision attendue, alpha, puissance et N analysable restent ouverts si absents.
...
Origines autorisées, distinctes : PARTICIPANT_REPORTED (déclaration), SITE_RECORDED (recueil clinique site), LAB_RESULT (résultat laboratoire), IMAGING_DERIVED (mesure quantitative issue de l'image), IMAGING_READER_RECORDED (qualification du lecteur), SYSTEM_DERIVED (calcul déterministe secondaire), UNSPECIFIED si réellement indécidable. source décrit le document/mesure d'origine et qui consigne, sans inventer un dossier ou lecteur imposé. Les données du site, du laboratoire et de lecture restent séparées ; un calcul relie ses entrées sans effacer leur origine.
Spécifications déductibles : transpose uniquement les unités, modalités, conditions, formules et dépendances des variables réellement présentes dans DATA_MANAGEMENT_CRF et le Project. Une dérivation conserve toutes ses entrées et leurs origines. Explicite une conversion d'unité nécessaire sans modifier le modèle scientifique. Une catégorisation ou un seuil non adopté reste ouvert. Aucun biomarqueur, mesure, formule ou critère d'un autre projet ne doit être introduit.
required précise oui, conditionnel ou nécessaire à l'analyse principale avec motif ; cela organise le recueil sans inventer rejet automatique ou imputation. condition conserve les règles existantes ; controls fournit des contrôles de type, cohérence d'unité, présence, appariement, temporalité et seuils déjà décidés. Pas de valeurs normales ni bornes cliniques non adoptées. La qualité peut rester un texte motivé en l'absence d'échelle ; ne crée aucun score. Un antécédent, une mesure, un traitement et son indication restent distincts : aucun diagnostic ou critère d'exclusion n'est déduit automatiquement de l'un d'eux.
...
ORGANISATION CRF : dérive les modules des variables et du parcours effectivement retenus : sélection, consentement si applicable, caractéristiques pertinentes, interventions ou observations, mesures, dérivations, qualité, éligibilité analytique et clôture. Aucun module scientifique n'est imposé par défaut. Regroupe les modules voisins sans perdre leurs fonctions. Les contrôles de processus restent présentés séparément, sans fausse variable canonique. Identifiants/pseudonymisation, autorisations, consentement et règles institutionnelles restent ouverts si non définis.
RECRUTEMENT = deux objets dans le même document : A. texte candidat court, clair, non coercitif, sans promesse de bénéfice ni résultat individuel ; B. formulaire de pré-screening utilisable avec blancs ____ et cases ☐, champs sur lignes séparées. Projette exactement les critères adoptés et leurs conjonctions, alternatives et conditions, avec oui/non/à confirmer et motif. Une incertitude ne vaut ni exclusion ni éligibilité automatique. Les critères médicaux nécessitent la vérification compétente ; ce formulaire ne vaut ni consentement, diagnostic ou donnée d'analyse. Coordonnées et règles de conservation restent ouvertes si absentes.
COHÉRENCE : population, effectif et son statut, critères, parcours, temporalités, mesures, endpoint, analyses et éléments ouverts sont identiques dans les quatre documents là où pertinents. Aucun ancien critère remplacé ne survit. Aucun paramètre scientifique, technique, institutionnel ou bibliographique absent n'est complété par défaut. Documents de travail à revoir : aucune validation scientifique, réglementaire ou institutionnelle revendiquée.`;
