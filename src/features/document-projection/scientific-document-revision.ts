...
const supported = (source: ProjectSource, assertion: RuntimeAssertion) => source.scientificWeight.assessment === "OWNER_QUALIFIED_ASSERTIONS"
  ...
  && !source.conflicts?.some((conflict) => ["CONTRADICTION", "CONTROVERSY"].includes(conflict.state) && conflict.positionIds.some((id) => [assertion.stableId, assertion.revision].includes(id)))
...
export const initialDocumentEvidence = (library: ProjectSourceLibrary): DocumentEvidenceContent => {
  const paragraphs = availableDocumentEvidence(library).slice(0, 3);
  return evidenceWithSources(library, { owner: "KNOWLEDGE", libraryDigest: library.digest, paragraphs, sources: [], excludedSourceRefs: [], emphasisSourceRef: null,
...
export const regenerateDocumentEvidence = (library: ProjectSourceLibrary, previous?: DocumentEvidenceContent): DocumentEvidenceContent => {
  if (!previous) return initialDocumentEvidence(library);
  ...
  const content = evidenceWithSources(library, { ...copy(previous), paragraphs,
...
const evidenceWithSources = (library: ProjectSourceLibrary, content: DocumentEvidenceContent): DocumentEvidenceContent => {
  const ids = new Set(content.paragraphs.flatMap((paragraph) => paragraph.sourceRefs));
...
export const documentEvidenceSections = (content: DocumentEvidenceContent): DocumentSectionInstance[] => {
  const refs = unique(content.paragraphs.flatMap((paragraph) => [...paragraph.assertionRefs, ...paragraph.sourceRefs]));
...
export const validateDocumentEvidence = (content: DocumentEvidenceContent) => {
  ...
  for (const paragraph of content.paragraphs) {
    ...
    for (const sourceId of paragraph.sourceRefs) {
      ...
      if (!paragraph.assertionRefs.every((ref) => source.assertions.some((assertion) => assertion.revision === ref && supported(source, assertion) && assertionText(assertion) === paragraph.text))) throw new Error("DOCUMENT_CITATION_ASSERTION_MISMATCH");
      refs.add(sourceId);
    }
  }
...
export const reviseScientificDocument = (input: {
  ...
  const admissible = all.filter((paragraph) => paragraph.sourceRefs.some((ref) => !content.excludedSourceRefs.includes(ref)));
  ...
  if (input.transformation === "REMOVE_SOURCE") {
    ...
    content.paragraphs = content.paragraphs.slice(0, Math.max(1, Math.ceil(content.paragraphs.length / 2))); content.depth = "SHORT";
  } else if (input.transformation === "CAUTION") content.caution = true;
  else {
    const target = sourceId ? admissible.filter((paragraph) => paragraph.sourceRefs.includes(sourceId)) : admissible;
  ...
  content = evidenceWithSources(library, content);
  validateDocumentEvidence(content);
