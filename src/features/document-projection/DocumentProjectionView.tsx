...
export default function DocumentProjectionView({ projection, history, onReturnToProject }: Props) {
  ...
  const statusCounts = projection.sections.reduce<Record<DocumentSectionStatus, number>>((counts, section) => ({ ...counts, [section.status]: counts[section.status] + 1 }), { GENERATABLE: 0, PARTIALLY_GENERATABLE: 0, BLOCKED: 0, NOT_GENERATABLE: 0, NOT_APPLICABLE: 0 });
  ...
  return <div className="mt-8 min-w-0" data-testid="document-projection-view">
    <Card className="border-primary/40">
      ...
      <div className="mt-5 flex flex-wrap gap-2">{(Object.keys(statusCounts) as DocumentSectionStatus[]).map((status) => <Badge key={status} warning={["BLOCKED", "NOT_GENERATABLE", "PARTIALLY_GENERATABLE"].includes(status)}>{statusLabels[status]} · {statusCounts[status]}</Badge>)}</div>
    ...
    <section className="mt-8" aria-labelledby="projection-decisions-title"><h3 id="projection-decisions-title" className="text-2xl font-bold">Décisions humaines et provenance</h3><div className="mt-4 grid gap-4 lg:grid-cols-2"><Card><h4 className="font-semibold">Registre des décisions</h4>{projection.humanDecisions.length ? projection.humanDecisions.map((item) => <div key={item.decisionId} className="mt-3 text-sm"><Badge warning={item.status !== "APPROVED"}>{item.status}</Badge><p className="mt-2 font-medium">{item.label}</p><p className="mt-1 text-muted-foreground">{item.actor ?? "Acteur non enregistré"} · mandat {item.mandate ?? "non enregistré"} · version {item.version}</p><p className="mt-1 text-muted-foreground">{item.reason}</p></div>) : <p className="mt-3 text-sm text-muted-foreground">Aucune décision transportée.</p>}</Card><Card><h4 className="font-semibold">Provenance globale</h4>{projection.provenanceRefs.map((item) => <p key={item} className="mt-2 break-all text-xs text-muted-foreground">• {item}</p>)}</Card></div></section>
