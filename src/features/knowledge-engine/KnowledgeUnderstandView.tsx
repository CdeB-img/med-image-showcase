import { BookOpen, CircleAlert, History, RotateCcw, Sparkles, Trash2 } from "lucide-react";
...
export default function KnowledgeUnderstandView({ result, sessionId, contextVersion, onClarify }: Props) {
  ...
  const currentCorpusDigests = useMemo(() => Object.fromEntries(result.provenance.map((item) => [item.providerId, item.representationDigest])), [result.provenance]);
  const refreshHistory = () => setHistory(loadKnowledgeSnapshots(window.localStorage, {
    registryDigest: result.registrySnapshotRef,
    providerVersions: result.providerVersions,
    corpusRepresentationDigests: currentCorpusDigests,
    question: result.request.originalQuestion,
    contextVersion,
  }));

  useEffect(() => {
    try {
      const snapshot = createKnowledgeSnapshot({ sessionId, contextVersion, result, projectionSettings: { depth, openDisclosure: "ANSWER" } });
  ...
  }, [contextVersion, currentCorpusDigests, depth, result, sessionId]);

  const displayedResult = selectedHistorical?.snapshot.result ?? result;
  ...
  return <div className="mt-8" aria-live="polite">
    ...
    <details className="mt-4 rounded-2xl border bg-card p-5">
      <summary className="cursor-pointer text-lg font-semibold focus-visible:ring-2 focus-visible:ring-ring">Preuves et sources</summary>
      <p className="mt-3 text-sm text-muted-foreground">Les sources, versions et localisateurs restent secondaires mais accessibles. Aucun identifiant technique d’exécution n’est nécessaire pour lire la réponse.</p>
