...
export const analyzeGaps = (
  ...
  if (referenceSourceSnapshots.some((snapshot) => ["METADATA_ONLY", "CONTENT_ACCESSIBLE_NOT_STORED", "LOCAL_DOCUMENT_AVAILABLE"].includes(snapshot.contentAvailability))) push("MISSING_SOURCE_ACCESS", "REFERENCE_DOCUMENT_CONTENT", "Au moins une source documentaire ne dispose pas de contenu section-indexé utilisable par le runtime ; aucun contenu spécifique n’en est inféré.", conceptIds, "Acquérir et indexer explicitement une copie autorisée et digestée.");
