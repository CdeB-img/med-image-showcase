...
const Tag = ({ children, tone = "neutral" }: { children: React.ReactNode; tone?: "neutral" | "good" | "warning" }) => <span className={`inline-flex max-w-full break-words rounded-full border px-2.5 py-1 text-xs font-medium ${tone === "good" ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-700 dark:text-emerald-200" : tone === "warning" ? "border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-200" : "border-border bg-muted text-muted-foreground"}`}>{children}</span>;
...
export default function ProtocolDesignerDemo() {
  ...
  const reset = () => {
    deleteSession(window.localStorage); setCandidate(null); setSession(createProtocolDesignerSession()); setQuestion(""); setInterpretation(null); setReviews({}); setCorrections({}); setReformulation(""); setAmbiguityResolutions({}); setContradictionResolutions({}); setAnswerDrafts({}); setError(null); setMajorChange(null); setKnowledgeOpen(false);
  };
